...
# produce density function for each row. Each column corresponds to a row
post.density <- 
...
```{r}
hyp.means <- density.formatted %>% select(Player, distr, density) %>% pivot_wider(id_cols = Player, names_from = "distr", values_from = "density", values_fn = list)

data.frame("clutch.prob" = apply(hyp.means, 1, function(x) mean(x$posterior > x$prior))) %>% 
  kable(
