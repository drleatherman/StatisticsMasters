...
A Bayesian Posterior is a distribution for $\theta | Y$ whereas the sampling distribution is for $\hat \theta$. While their expected values are both $\mu$, the sampling distribution is not a distribution of $\theta$ hence why "Confidence" is used when in the interpretation. The Bayesian Posterior is a distribution of $\theta$ so the posterior can be used for the interpretation.
#+end_quote
